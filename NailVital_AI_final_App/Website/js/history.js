...
const History = {
  ...
  async generateHtmlPdfFallback(scanRecords, filename) {
    const user = Auth.currentUser || { name: 'Priya Sharma', email: 'priya@example.com' };
    
    // Pre-fetch images and convert to base64 to avoid all CORS/loading issues in html2canvas
    ...
    const recordsWithImages = await Promise.all(scanRecords.map(async scan => {
      const b64 = scan.image_path ? await getBase64(`${CONFIG.API_BASE_URL}/${scan.image_path}`) : await getBase64('https://images.unsplash.com/photo-1599839619722-39751411ea63?w=200&h=200&fit=crop');
      return { ...scan, b64Image: b64 };
    }));

    const recordsHtml = recordsWithImages.map((scan, idx) => `
      <div style="border: 1px solid #CBD5E1; border-radius: 12px; padding: 20px; margin-bottom: 20px; background: #FFFFFF; page-break-inside: avoid;">
        ...
        <div style="text-align: center; margin: 15px 0;">
          ${scan.b64Image ? `<img src="${scan.b64Image}" style="width: 120px; height: 120px; border-radius: 8px; object-fit: cover; border: 1px solid #E2E8F0;">` : `<div style="width: 120px; height: 120px; border-radius: 8px; background: #F1F5F9; display: inline-block; line-height: 120px; color: #94A3B8; font-size: 12px; border: 1px solid #E2E8F0;">Image Unavailable</div>`}
        </div>
    ...
    const fullHtml = `
      <div style="padding: 30px; background: #FFFFFF; color: #0F172A; font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; width: 700px; box-sizing: border-box;">
    ...
    renderContainer.style.width = '700px'; // Force width to fix cropping
